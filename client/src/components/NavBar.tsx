...
import { changeCountry, changeGenre, changeYear } from '../store/reducers/FilterSlice';
...
export const NavBar = () => {
  const { movies, isLoading, error } = useAppSelector((store) => store.ListReducer);
  ...
  useEffect(() => {
    if (inputName.length > 0) {
      dispatch(fetchListMovies(inputName));
    } else {
      dispatch(listCleaner());
    }
  }, [inputName]);
  useEffect(() => {
    if (inputName && movies.docs.length < 1) {
      setEmpty(true);
    } else {
      setEmpty(false);
    }
  ...
  return (
    <div className="header">
      ...
      <div>
        <ul className="header__menu">
          <li className="header__menu-item">
            <Link to="home" className={location.pathname.includes('home') ? 'active' : ''}>
              Профиль
            </Link>
          </li>
          <li className="header__menu-item">
            <Link
              to={'/collection'}
              className={location.pathname.includes('collection') ? 'active' : ''}>
              Подбороки
            </Link>
          </li>
          <li className="header__menu-item">
          ...
          <li className="header__menu-item">
            <Link
              ...
              className={location.pathname.includes('movie', 11) ? 'active' : ''}
          ...
          <li className="header__menu-item">
            <Link
              ...
              className={location.pathname.includes('tv-series', 11) ? 'active' : ''}
              onClick={resetFilters}>
              Сериалы
            </Link>
          </li>
          <li className="header__menu-item">
            <Link
              ...
              className={location.pathname.includes('cartoon', 11) ? 'active' : ''}
              onClick={resetFilters}>
              Мультфильмы
            </Link>
          </li>
          <li className="header__menu-item">
            <Link
              ...
              className={location.pathname.includes('anime', 11) ? 'active' : ''}
              onClick={resetFilters}>
              Аниме
            </Link>
          </li>
          <li className="header__menu-item">
            <Link to={'/actors'} className={location.pathname.includes('actors') ? 'active' : ''}>
              Актеры
            </Link>
          </li>
        </ul>
      ...
      <div className="header__input">
        <MyInput placeholder="Поиск по сайту" onSearch={setInputName} />
        {empty && (
